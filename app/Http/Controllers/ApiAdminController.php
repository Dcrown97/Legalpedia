    ...
    public function allJudgement(Request $request)
    {
        ...
        try {
            ...
            if ($request->filled('id') && !$request->filled('year')) {
                ...
                $selected_court = [];
                $selected_court['court_id'] = $request->id;
                return response(['judgement_summaries' => $judgement_summaries, 'judgement_count' => $judgement_count, 'categories' => $categories, 'selected_court' => $selected_court, 'area_of_laws' => $area_of_laws, 'selected_year' => $selected_year]);
            }
            if (!$request->filled('id') && $request->filled('year')) {
                $judge = $judgement_summary->where('judgement_date', 'LIKE', '%' . $request->year . '%');
                $judgement_count =  $judge->count();
