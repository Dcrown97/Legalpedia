...
class AdminUserController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if (Auth::user()->role->name == 'Admin') {
            ...
            $search_param = $request->search_customer;

            (!is_null($request->start_date) && !is_null($request->end_date)) ? $dateSearchParams = true : $dateSearchParams = false;
            (!is_null($request->last_seen_start_date) && !is_null($request->last_seen_end_date)) ? $dateSeenParams = true : $dateSeenParams = false;
            // dd($request->last_seen_start_date, $request->last_seen_end_date);
            ...
            $user_count = User::when($status, function ($query, $status) {
                ...
                return $query->where('name', 'LIKE', '%' . $search_param . '%')
                    ->orWhere('surname', 'LIKE', '%' . $search_param . '%')
                ...
                return $query->whereBetween(DB::raw('DATE(active_date)'), [$startDate, $endDate]);
            })->when($dateSeenParams, function ($query) use ($request) {
                $last_seen_start_date = Carbon::parse($request->last_seen_start_date)->addDays(1);
                $last_seen_end_date = Carbon::parse($request->last_seen_end_date)->addDays(1);
                return $query->whereBetween(DB::raw('DATE(last_seen)'), [$last_seen_start_date, $last_seen_end_date]);
            })->count();
