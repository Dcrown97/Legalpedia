...
@section('content')
                    ...
                    <div class="tab-pane fade show active" id="all" role="tabpanel" aria-labelledby="all-tab">
                                    ...
                                    <div class="col-12 col-lg-6 col-xl">
                                        <div class="form-group">
                                            ...
                                            <select name="status" class="form-select mr-8">
                                                <option value="active" {{$selected_status['status'] == 'active' ? 'selected' : ''}}>Active</option>
                                                <option value="inactive" {{$selected_status['status'] == 'inactive' ? 'selected' : ''}}>Expired</option>
                                                <option value="null" {{$selected_status['status'] == 'null' ? 'selected' : ''}}>Do not have a package</option>
                                            </select>
                                        </div>
                                    </div>
                                    <div class="col-12 col-lg-6 col-xl">
                        ...
                        <div class="card" data-list='{"valueNames": ["orders-order", "orders-product", "orders-date", "orders-total", "orders-status", "orders-method"], "page": 10, "pagination": {"paginationClass": "list-pagination"}}' id="contactsList">
                                ...
                                <div class="card-header">
                                    <form action="{{route('admin.messages')}}" method="GET" class="d-flex">
                            ...
                            <form action="{{route('send.messages')}}" method="POST" class="d-flex">
                                @csrf
                                    <div class="col-auto">
                                    ...
                                    <div class="col-auto">
                                        <div class="form-check" style="margin-top: 10px">
                                            <input class="form-check-input list-checkbox-all" name="checkBoxArray" value="check all" id="ordersSelectAll" onclick="checkBoxAllById('ordersSelectOne', this)" type="checkbox">
                                            <label class="form-check-label" for="ordersSelectAll">&nbsp;</label>Send to all Customers
                                        </div>
                                    </div>
                                    <div class="col-auto me-n3">
                                        <input type="hidden" name="data" value="{{json_encode($data_array)}}">
                                        <input type="hidden" name="users" value="{{json_encode($user_array)}}">
                                        <button type="submit" name="send_single_message" id="single_message" onclick="this.classList.toggle('button--loading')" class="btn button_load text-white w-100 btn-primary">
                                            <span class="button__text"><i class="mdi mdi-send"></i> Send</span>
                                        </button>
                                        <button type="submit" name="send_multiple_message" id="multiple_message" onclick="this.classList.toggle('button--loading')" class="btn button_load text-white w-100 btn-primary">
                                            <span class="button__text"><i class="mdi mdi-send"></i> Send</span>
                                        </button>
                                    </div>
                                </div>
                                <div class="table-responsive">
                                    ...
                                    @if(count($users) > 0)
                                        <table class="table table-sm table-nowrap card-table">
                                            ...
                                            <tbody class="list" id="ordersSelectOne">
                                                @foreach($users as $user)
                                                    <tr>
                                                        <td>
                                                            <div class="form-check mb-n2">
                                                                <input class="form-check-input list-checkbox" type="checkbox" name="checkBoxArray[]" value="{{$user->id}}">
                                                                <label class="form-check-label" for="ordersSelectOne">&nbsp;</label>
                                                            </div>
                                                        </td>
                                                        <td class="orders-order">{{$user_no}}</td>
                                                        <?php $user_no++; ?>
                                                        <td class="orders-product">{{$user->name}} {{$user->surname}}</td>
                                        ...
                                        <tbody class="list">
                                            @foreach($all_messages as $message)
                                                <tr>
                                                    ...
                                                    @php
                                                        $receipients = App\Models\MailMessage::where('message_id', $message ? $message->id : '')->get();
                                                        $receipient = App\Models\MailMessage::where('message_id', $message ? $message->id : '')->orderBy('created_at', 'DESC')->first();
                                                        if($receipient) {
                                                            $receipient_no = 0;
                                                            $receipient_count = $receipients->count();
                                                            foreach($receipients as $receipient) {
                                                                $receipient_users['user_id'] = json_decode($receipient->users);
                                                                $receipient_no = $receipient_no + count($receipient_users['user_id']);
                                                            }
                                                        }
                                                    @endphp
                                                    @if($receipient)
                                                        <td class="orders-total">{{number_format($receipient_no)}}</td>
                                                        <td class="orders-total">{{number_format($receipient_count)}}</td>
                                                        @else
                                                        <td class="orders-total">--</td>
                                                        <td class="orders-total">--</td>
                                                    @endif
    ...
    <script>
        ...
        $(document).ready(function () {
            $("#multiple_message").hide();
            $('#ordersSelectAll').change(function () {
                if (this.checked) {
                    $("#multiple_message").show();
                    $("#single_message").hide();
                }
                else {
                    $("#multiple_message").hide();
        ...
        function checkBoxAllById(divId, event){
