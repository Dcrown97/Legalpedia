...
@section('title')
    <title>Legal Prompts - Legalpedia</title>
@endsection
...
@section('content')
            ...
            <div class="header-body">
                    ...
                    <div class="col-auto">
                        @if (Auth::user()->role->name == 'Admin')
                            <a href="#" class="btn btn-primary text-white" data-bs-toggle="modal"
                                ...
                                <i class="fe fe-plus"></i> Add Prompts
                            </a>
                        @endif
                ...
                @if (Auth::user()->role->name == 'Admin')
                    <div class="row align-items-end justify-content-end mt-4 p-3">
                        <form action="{{ route('admin.legal-prompts') }}" method="GET" class="me-3 d-flex">
                            <select name="category" class="form-select mr-8" data-choices='{"searchEnabled": true}'>
                                @foreach ($categories as $category)
                                    <option value="{{ $category->name }}"
                                        {{ $category->name == $selected_category['category'] ? 'selected' : '' }}>
                            ...
                            <a href="{{ url('admin/legal-prompts') }}" onclick="this.classList.toggle('button--loading')"
                ...
                @else
                    @if ($categories->prompt_cat)
                        <div class="row align-items-end justify-content-end mt-4 p-3">
                            <form action="{{ route('admin.legal-prompts') }}" method="GET" class="me-3 d-flex">
                                @php
                                    $all_categories = json_decode($categories->prompt_cat);
                                @endphp
                                <select name="category" class="form-select mr-8" data-choices='{"searchEnabled": true}'>
                                    @foreach ($all_categories as $category)
                                        @php
                                            $main_category = App\Models\LegalPromptCategory::where(
                                                'name',
                                                $category,
                                            )->first();
                                        @endphp
                                        <option value="{{ $main_category->name }}"
                                            {{ $main_category->name == $selected_category['category'] ? 'selected' : '' }}>
                                ...
                                <a href="{{ url('admin/legal-prompts') }}"
                                    onclick="this.classList.toggle('button--loading')"
                                    class="btn button_load text-white btn-primary btn-sm p-2 hide-mobile">
                                    <span class="button__text"><i class="mdi mdi-close"></i> Clear</span>
                                </a>
            ...
            <div class="col-12">
                <div class="card"
                    ...
                    <div class="card-header">
                        <h4 class="card-header-title">Prompts</h4>
                        <h4>{{ number_format($prompt_count) }} records</h4>
                    </div>
                    <div class="card-header">
                        <form>
                            <div class="input-group input-group-flush input-group-merge input-group-reverse">
                                <input class="form-control list-search" type="search" placeholder="Search prompts">
                                ...
                                @foreach ($prompts as $prompt)
                                    <li class="list-group-item">
                                        <div class="row align-items-center">
                                            <div class="col">
                                                <a data-toggle="collapse" href="#multiCollapse{{ $prompt->id }}"
                                                    role="button" aria-expanded="false"
                                                    aria-controls="multiCollapse{{ $prompt->id }}">
                                                    <h4 class="mb-1 item-name">
                                                        <span
                                                            class="mr-2 text-color">{{ Str::limit($prompt->prompt, 1, '') }}</span><span
                                                            class="text-gray">{{ $prompt->prompt }}</span>
                                                    </h4>
                                                </a>
                                                <div class="">
                                                    <div class="collapse multi-collapse"
                                                        id="multiCollapse{{ $prompt->id }}">
                                                        <div class="mt-2 p-3" style="line-height: 25px; font-weight: 400">
                                                            <p>{!! $prompt->desciption !!}</p>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                                                ...
                                                <div class="col-auto">
                                                    <div class="dropdown">
                                                        ...
                                                        <div class="dropdown-menu dropdown-menu-end">
                                                            <a href="{{ route('edit.prompt', $prompt->id) }}"
                                                                class="dropdown-item">
                                                                <i class="mdi mdi-pencil mr-2"></i> Edit
                                                            </a>
                                                            <form action="/admin/legal-prompts/{{ $prompt->id }}"
                                                                method="POST">
                                                                {{ csrf_field() }}
                                                                {{ method_field('DELETE') }}
            ...
            <div class="modal-dialog modal-fullscreen p-9">
                <div class="modal-content rounded">
                    <div class="modal-header">
                        <div class="fs-1 fw-boldest">Add new prompt</div>
                        ...
                        <div class="stepper stepper-links d-flex flex-column" id="kt_modal_create_project_stepper">
                            <div class="container">
                                <div class="stepper-nav justify-content-center">
                                    <form action="{{ route('store.prompt') }}" method="POST">
                                        @csrf
                                        ...
                                        <div class="form-group">
                                            ...
                                            <textarea name="description" id="summernote" rows="5" class="form-control" placeholder="Enter content"></textarea>
                                        </div>
                                        <div class="form-group">
                                            <label class="form-label mb-1">
                                                Category
    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            function fetch_prompt(page) {
                $.ajax({
                    url: "/admin/legal-prompts/p?page=" + page,
                    success: function(prompts) {
                        $('#table_data').html(prompts);
                    }
                });
            }
        });
