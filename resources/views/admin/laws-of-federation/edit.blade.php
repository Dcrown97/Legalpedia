...
@section('content')
    ...
    <style>
        ...
        .post_list_uls li:hover {
            list-style: none;
            background: #000000;
            color: #fff;
        }

        .post_list_uls li.ui-state-highlight {
            padding: 20px;
            background-color: #eaecec;
            border: 1px dotted #ccc;
            cursor: move;
            ...
            <div class="col-12 col-lg-12 col-xl-12">
                <div class="card">
                    <div class="card-body p-5">
                        <form class="tab-content pb-4" id="wizardSteps" action="{{ route('update.fed', $fed->id) }}"
                            method="POST">
                            {{ csrf_field() }}
                            {{ method_field('patch') }}
                            <div class="tab-pane fade show active" id="wizardStepOne" role="tabpanel"
                                ...
                                <div class="form-group">
                                    ...
                                    <textarea name="description" id="summernote" rows="5" class="form-control custom-textarea"
                                        placeholder="Enter description">{{ $fed->description }}</textarea>
                                </div>
                                <div class="form-group">
                                    <label class="form-label mb-1">
                                        Category
                                ...
                                <div class="form-group">
                                    ...
                                    <textarea name="subsidiary_legislation" id="summernote1" rows="5" class="form-control custom-textarea"
                                        placeholder="">{{ $fed->subsidiary_legislation }}</textarea>
                                </div>
                                <hr class="my-5">
                            ...
                            <div class="tab-pane fade" id="wizardStepTwo" role="tabpanel" aria-labelledby="wizardTabTwo">
                                ...
                                @if (count($fed_parts) > 0)
                                    @php
                                        $fed_part_no = 1;
                                        $last_part = App\Models\LawOfFedPart::where('law_of_federation_id', $fed->id)
                                            ->orderBy('id', 'DESC')
                                            ->first();
                                        $part_count = App\Models\LawOfFedPart::where(
                                            'law_of_federation_id',
                                            $fed->id,
                                        )->count();
                                        $id_start = 3;
                                    @endphp
                                    <ul id="post_sortable" class="post_list_ul">
                                        @foreach ($fed_parts as $fed_part)
                                            <li class="ui-state-default" data-id="{{ $fed_part->id }}">
                                                <div class="form-group">

                                                    <span class="pos_num">{{ $loop->index + 1 }}</span>
                                                    <span>{{ $fed_part->title }} Part Title</span>

                                                    <input type="text" name="part_header[{{ $fed_part->id }}][]"
                                                        class="form-control" value="{{ $fed_part->part_header }}">
                                                </div>
                                                <hr class="my-5">
                                                @php
                                                    $get_fed_sections = App\Models\LawOfFedSection::where(
                                                        'law_of_fed_part_id',
                                                        $fed_part ? $fed_part->id : '',
                                                    )
                                                        ->where('law_of_federation_id', $fed->id)
                                                        ->orderBy('position', 'asc')
                                                        ->get();
                                                    $fed_section_no = 1;
                                                    $last_section = App\Models\LawOfFedSection::where(
                                                        'law_of_fed_part_id',
                                                        $fed_part ? $fed_part->id : '',
                                                    )
                                                        ->where('law_of_federation_id', $fed->id)
                                                        ->orderBy('id', 'DESC')
                                                        ->first();
                                                    $section_count = App\Models\LawOfFedSection::where(
                                                        'law_of_fed_part_id',
                                                        $fed_part ? $fed_part->id : '',
                                                    )
                                                        ->where('law_of_federation_id', $fed->id)
                                                ...
                                                @if (count($get_fed_sections) > 0)
                                                    <ul class="post_list_uls section_sortables">
                                                        @foreach ($get_fed_sections as $fed_section)
                                                            <li class="ui-state-default"
                                                                data-id="{{ $fed_section->id }}">
                                                                <div class="add_more">
                                                                    <div class="form-group">
                                                                        <span
                                                                            class="pos_nums">{{ $loop->index + 1 }}</span>
                                                                        <span>{{ $fed_section->title }} Section
                                                                            Header</span>
                                                                        <input type="text"
                                                                            name="part_header[{{ $fed_part->id }}][10][{{ $fed_section->id }}][]"
                                                                            class="form-control"
                                                                            value="{{ $fed_section->section_header }}">
                                                                    </div>
                                                                    <div class="form-group">
                                                                        <label class="form-label mb-1">
                                                                            Section Body
                                                                        </label>
                                                                        <textarea name="part_header[{{ $fed_part->id }}][10][{{ $fed_section->id }}][]" rows="5"
                                                                            id="summernote{{ $id_start }}" class="custom-textarea form-control">{{ $fed_section->section_body }}</textarea>
                                                                        @php
                                                                            $id_start++;
                                                                        @endphp
                                                                    </div>
                                                                    <div class="form-roup mb-4">
                                                                        <div class="justify-content-end">
                                                                            <input type="hidden"
                                                                                name="remove_fed_section_id"
                                                                                value="{{ $fed_section->id }}">
                                                                            <span
                                                                                onclick="removeSection('{{ $fed_section->id }}')"
                                                                                class="cursor-pointer text-color"><i
                                                    ...
                                                    <input type="hidden" id="secton-no{{ $fed_part->id }}"
                                                        value="{{ $section_count }}">
                                                    <input type="hidden" id="secton-id{{ $fed_part->id }}"
                                                        value="{{ $last_section->id }}">
                                                    <div class="justify-content-end">
                                                        <a type="button" id="more_fields{{ $fed_part->id }}"
                                                            class="text-color"
                                                            onclick="addFields('add_field{{ $fed_part->id }}', '{{ $fed_part->id }}', 'secton-no{{ $fed_part->id }}', 'secton-id{{ $fed_part->id }}')"><i
                                                ...
                                                <div class="form-roup mb-4">
                                                    <div class="justify-content-end">
                                                        <input type="hidden" name="fed_part_id"
                                                            value="{{ $fed_part->id }}">
                                                        <span class="text-color cursor-pointer"
                                                            onclick="removePart('{{ $fed_part->id }}')"><i
                                    ...
                                    <div class="justify-content-end mb-5">
                                        ...
                                        <a type="button" id="more_part" class="text-color"
                                            onclick="addPart('part-no')"><i class="mdi mdi-plus"></i> Add New Part</a>
                                    </div>
                                @else
                                    <div id="add_new_part"></div>
                                    <div class="justify-content-end mb-5">
                                        <a type="button" id="more_new_part" class="text-color"
                                            onclick="addNewPart()"><i class="mdi mdi-plus"></i> Add New Part</a>
                                    </div>
                                @endif
                                <div class="nav row align-items-center">
                                    <div class="col-auto">
                            ...
                            <div class="tab-pane fade" id="wizardStepThree" role="tabpanel"
                                    ...
                                    @foreach ($fed_scheds as $fed_sched)
                                        <div class="add_more">
                                            <div class="form-group">
                                                ...
                                                <input type="hidden" name="sched[{{ $fed_sched->id }}][]"
                                                    value="{{ $fed_sched->law_of_federation_id }}">
                                                <input type="text" name="sched[{{ $fed_sched->id }}][]"
                                                    class="form-control" value="{{ $fed_sched->sched_header }}">
                                            </div>
                                            <div class="form-group">
                                                <label class="form-label mb-1">
                                                    Schedule Body
                                                </label>
                                                <textarea name="sched[{{ $fed_sched->id }}][]" id="summernote{{ $sched_id }}"
                                                    class="custom-textarea form-control" rows="5">{{ $fed_sched->sched_body }}</textarea>
                                                @php
                                                    $sched_id++;
                                                @endphp
                                            </div>
                                            <div class="form-roup mb-4">
                                                <div class="justify-content-end">
                                                    <input type="hidden" name="fed_sched_id"
                                                        value="{{ $fed_sched->id }}">
                                                    <button type="submit" name="remove_sched"
                                ...
                                <div class="justify-content-end">
                                    <a type="button" id="more_scheds" class="text-color" onclick="addScheds()"><i
                                            class="mdi mdi-plus"></i> Add Schedule</a>
                                </div>
                                <hr class="my-5">
                                <div class="nav row align-items-center">
                                    <div class="col-auto">
    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            var sched_id = 300;
            // Loop through $fed_scheds to initialize Summernote
            @foreach ($fed_scheds as $fed_sched)
                $('#summernote' + sched_id).summernote({
                    height: 200
                });
                sched_id++;
            @endforeach
            ...
            $(".section_sortables").sortable({
                placeholder: "ui-state-highlight",
                update: function(event, ui) {
                    var post_order_ids = [];
                    $('.section_sortables li').each(function() {
                        post_order_ids.push($(this).data("id"));
                    });
                    console.log(post_order_ids);
                    $.ajax({
                        type: "POST",
                        url: "{{ route('fedSection.order_change') }}",
                        dataType: "json",
                        data: {
                            order: post_order_ids,
                            _token: "{{ csrf_token() }}"
                        ...
                        success: function(response) {
                            ...
                            $('.section_sortables li').each(function(index) {
                                $(this).find('.pos_nums').text(index + 1);
                            });
                            window.location.href = "{{ url()->current() }}";
                        },
                        error: function(xhr, status, error) {
        ...
        initSummernoteOnElement('summernote50');
        ...
        function addFields(addField, part_no, sectionNo, section_id) {
            ...
            console.log('first', newSecTextAreaId)
            ...
            divcreate.innerHTML = '<div class="form-group"><label class="form-label mb-1">' + section_no +
                '. Section Header</label><input type="hidden" name="fed_section_part_id" value="' + part_no +
                '"><input type="text" name="section_part_header[' + part_no + '][10][' + sectionId +
                '][]" class="form-control"></div><div class="form-group"><label class="form-label mb-1">Section Body</label> <textarea class="form-control" name="section_part_header[' +
                part_no + '][10][' + sectionId + '][]" rows="5" id="' + newSecTextAreaId +
        ...
        function addPart(part_No) {
            ...
            divcreate.innerHTML = '<div class="form-group"><label class="form-label mb-1">' + part_no +
                '. Part Title</label><input type="text" name="new_part_header[' + part_no +
                '][]" class="form-control"></div><hr class="my-5"><div class="form-group"><label class="form-label mb-1">' +
                section_noss + '. Section Header</label><input type="text" name="new_part_header[' + part_no + '][10][' +
                section_nos +
                '][]" class="form-control"></div><div class="form-group"><label class="form-label mb-1">Section Body</label> <textarea class="form-control" name="new_part_header[' +
        ...
        function addFieldss(addField, section_id, partNo, section_no) {
            ...
            divcreate.innerHTML = '<div class="form-group"><label class="form-label mb-1">' + sectionNo +
                '. Section Header</label><input type="text" name="new_part_header[' + partNo + '][10][' + sectionId +
                '][]" class="form-control"></div><div class="form-group"><label class="form-label mb-1">Section Body</label> <textarea class="form-control" name="new_part_header[' +
        ...
        // Initialize the first Summernote
        initSummernoteOnElement('summernote200');

        function addNewPart() {
            new_part_no++;
            addPartSummernote++;
            var addPartTextAreaId = 'summernote' + addPartSummernote;
            ...
            divcreate.innerHTML = '<div class="form-group"><label class="form-label mb-1">' + new_part_no +
                '. Part Title</label><input type="text" name="part_header[' + new_part_no +
                '][]" class="form-control"></div><hr class="my-5"><div class="form-group"><label class="form-label mb-1">' +
                new_section_noss + '. Section Header</label><input type="text" name="part_header[' + new_part_no +
                '][10][' + new_section_nos +
                ...
                new_part_no + '][10][' + new_section_nos +
                '][]" rows="5" id="' + addPartTextAreaId + '"></textarea></div><hr class="my-5"><div id="add_field' + new_part_no +
                '"></div><div class="justify-content-end"><a type="button" id="more_fields' + new_part_no +
                '" class="text-color" onclick="addNewField(`add_field' + new_part_no + '`, `' + new_section_nos + '`, `' +
            new_part_no + '`, `' + new_section_noss +
            '`)"><i class="mdi mdi-plus"></i>Add Section</a></div><hr class="my-5">';
            objTo.appendChild(divcreate);
            initSummernoteOnElement(addPartTextAreaId);
            initMCEall();
        }
        ...
        // Initialize the first Summernote
        initSummernoteOnElement('summernote251');

        function addNewField(addField, sectionNo, partNo, sectionNoss) {
            sectionNo++;
            sectionNoss++;
            addPartSecSummernote++;
            var addPartSecTextAreaId = 'summernote' + addPartSecSummernote;
            var objTo = document.getElementById(addField)
            var divcreate = document.createElement("div");
            divcreate.innerHTML = '<div class="form-group"><label class="form-label mb-1">' + sectionNoss +
                '. Section Header</label><input type="text" name="part_header[' + partNo + '][10][' + sectionNo +
                '][]" class="form-control"></div><div class="form-group"><label class="form-label mb-1">Section Body</label> <textarea class="form-control" name="part_header[' +
                partNo + '][10][' + sectionNo + '][]" rows="5" id="' + addPartSecTextAreaId + '"></textarea></div><hr class="my-5">';
            objTo.appendChild(divcreate);
            initSummernoteOnElement(addPartSecTextAreaId);
            initMCEall();
        }



        <?php $fed_sched = App\Models\LawOfFedSched::where('law_of_federation_id', $fed->id)
            ->orderBy('id', 'DESC')
            ->first(); ?>
        ...
        function addScheds() {
            ...
            divcreate.innerHTML = '<div class="form-group"><label class="form-label mb-1">' + sched_no +
                '. Schedule Header</label><input type="hidden" name="new_sched[' + sched_id +
                '][]" value="{{ $fed->id }}"><input type="text" name="new_sched[' + sched_id +
                '][]" class="form-control"></div><div class="form-group"><label class="form-label mb-1">Schedule Body</label> <textarea class="form-control custom-textarea" name="new_sched[' +
                sched_id + '][]" rows="5" id="' + newSchedsTextAreaId + '"></textarea></div><hr class="my-5">';
            objTo.appendChild(divcreate);
